...
const TrackForm = () => {
  ...
  const uploadImage = (e) => {
    ...
    console.log(image.selectedDocument.file[0].name)
    ...
    uploadTask.on("state_changed",
      ...
      () => {
        getDownloadURL(uploadTask.snapshot.ref).then(async (imageURL) => {
          const audioRef = ref(storage, `audios/${audio.selectedDocument.file[0].name}`)

          uploadBytesResumable(audioRef, audio.selectedDocument.file[0])
          const uploadTask = uploadBytesResumable(audioRef, audio.selectedDocument.file[0]);
          uploadTask.on("state_changed",
            (snapshot) => {
              const progress =
                Math.round((snapshot.bytesTransferred / snapshot.totalBytes) * 100);
  ...
  return (
    <div className=" absolute p-3 top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2  bg-white border border-1 border-solid border-blue-500 rounded-md  hidden">
      
      <form  onSubmit={uploadImage}>
        <h3 className='font-semibold text-center mb-5'>Upload a new track</h3>
        ...
        />
        <button className="bg-blue-200 p-2 hover:bg-blue-300 text-gray-700">Add Track</button>
